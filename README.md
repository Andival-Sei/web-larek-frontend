...
2. `Presenter` (см. `src/index.ts`) ловит событие, создаёт компонент «детали товара» (расширенную `Card`) с переданным объектом товара, вставляет его в контент `Modal` и открывает окно.
